...
function updatePrice(){
  const glazingDropdown = document.getElementById('glazing');//gets the glazing option
  const glazing = glazingDropdown.options[glazingDropdown.selectedIndex].text;
  console.log(glazing)
  const glazingPrice= glazingType[glazing];// gets the numerical value of the selected option in the glazing dropdown
...
function addToCart() {
  const glazingDropdown = document.getElementById('glazing');//gets the glazing option
  const glazing = glazingDropdown.options[glazingDropdown.selectedIndex].text;//sets glazing to name
  const packDropdown = document.getElementById('packsize');
  const packSize = packDropdown.options[packDropdown.selectedIndex].text;//sets glazing to text 
  ...
  localStorage.setItem('cart', JSON.stringify(cart));//store the current state of the cart in localStorage as a JSON string
...
window.onload = function () {
  cart = JSON.parse(localStorage.getItem('cart')) || [];//retrieve cart from local storage 
